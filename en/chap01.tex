...
Important thing to notice in this section is that the universe $S$ has no restrictions. The models trained by us use $S$ as a subset of $\mathbb{R}^n$ in order to allow working with neural networks. 
...
\begin{proof}
	...
	\begin{itemize}
		...
		\footnotetext{Assuming $\varphi$ does not contain $x$ as a variable. Otherwise we have to rename the variable $x$ in $\varphi$. This does not break eqisatisfiability.}
	\end{itemize}
...
The XOR function is also equivalent to addition modulo 2. In \Autoref{section:addition example} we show our handcrafted networks that simulate exactly addition modulo n for arbitrary positive n.
